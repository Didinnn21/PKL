...
<head>
    ...
    <style>
        ...
        .btn-primary {
            @apply inline-flex items-center px-4 py-2 text-sm font-medium text-black bg-yellow-500 rounded-lg hover:bg-yellow-600 transition;
        }

        .btn-secondary {
            @apply inline-flex items-center px-4 py-2 text-sm font-medium text-white bg-gray-800 rounded-lg hover:bg-gray-700 transition;
        }
    </style>
</head>
...
<body class="bg-black">
    {{-- Navigation Bar --}}
    <div x-data="{ navOpen: false, scrolled: false, atTop: true, init() { window.addEventListener('scroll', () => { this.scrolled = window.scrollY > 50; this.atTop = window.scrollY <= 50; }); } }" :class="{ 'bg-black bg-opacity-70 backdrop-blur-md': scrolled, 'bg-transparent': atTop }" class="fixed top-0 left-0 right-0 z-50 transition-all duration-300">
        <div class="container mx-auto px-4 sm:px-6">
            <div class="flex items-center justify-between h-20">
                ...
                <nav class="hidden sm:flex items-center space-x-8">
                    ...
                    <a href="/register" class="text-gray-300 hover:text-white transition font-medium text-sm">CUSTOM ORDER</a>
                    <a href="#tentang-kami" class="text-gray-300 hover:text-white transition font-medium text-sm">TENTANG KAMI</a>
                    <a href="#faq" class="text-gray-300 hover:text-white transition font-medium text-sm">FAQ</a>
                </nav>

                {{-- Login Button and Mobile Menu --}}
                <div class="flex items-center">
                    <a href="/login" class="hidden sm:inline-flex btn-primary">Login</a>
        ...
        <div x-show="navOpen" @click.away="navOpen = false" x-transition class="sm:hidden bg-black bg-opacity-90">
            <div class="flex flex-col items-center space-y-4 py-6">
                <a href="#koleksi" @click="navOpen = false" class="text-gray-300 hover:text-white transition">KOLEKSI</a>
                <a href="/register" @click="navOpen = false" class="text-gray-300 hover:text-white transition">CUSTOM ORDER</a>
                <a href="#tentang-kami" @click="navOpen = false" class="text-gray-300 hover:text-white transition">TENTANG KAMI</a>
    ...
    <main>
        ...
        <section class="relative h-screen flex items-center justify-center overflow-hidden">
            <div class="absolute inset-0 z-0">
                <img src="{{ asset('images/BG.jpg') }}" alt="Hero Background" class="w-full h-full object-cover opacity-40">
                <div class="absolute inset-0 bg-gradient-to-t from-black via-transparent to-black"></div>
            </div>
            <div class="relative z-10 text-center text-white px-4">
                <h1 class="text-4xl sm:text-6xl md:text-7xl font-extrabold mb-4 leading-tight">Kestore.id</h1>
            ...
            <div class="container mx-auto px-4 sm:px-6">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-12 items-center">
                    <div class="reveal-card">
                        <img src="{{ asset('images/P-Hoodie.jpg') }}" alt="Tentang Kestore.id" class="rounded-2xl shadow-lg w-full h-auto object-cover">
                    </div>
                    <div class="reveal-card">
                        <h2 class="text-3xl sm:text-4xl font-bold text-white mb-6">Tentang Kestore.id</h2>
                        <p class="text-gray-400 mb-4">Kestore.id adalah brand lokal custom apparel yang berdiri sejak 2021. Didirikan oleh Deki Muhamad F.R., Kestore.id menghadirkan kaos, hoodie, dan crewneck dengan desain bebas sesuai gaya kamu.</p>
                        <p class="text-gray-400">Dengan bahan premium dan sablon berkualitas, kami siap bantu kamu tampil beda mulai dari pesanan satuan sampai produksi massal.</p>
                    </div>
                </div>
            </div>
        </section>
            ...
            <div class="container mx-auto px-4 sm:px-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                    @foreach ($products as $product)
                        <div class="reveal-card bg-gray-900 rounded-2xl overflow-hidden shadow-lg transform hover:-translate-y-2 transition-transform duration-300 group">
                            <div class="relative overflow-hidden h-72">
                                <img src="{{ asset('images/product/' . $product->image) }}" alt="{{ $product->name }}" class="w-full h-full object-cover transition-transform duration-500 group-hover:scale-110">
                            </div>
                            <div class="p-6">
                                ...
                                <div class="flex items-center justify-between">
                                    ...
                                    <a href="{{ route('product.detail', $product->id) }}" class="btn-primary">Lihat Detail</a>
                                </div>
                            </div>
                        </div>
                    @endforeach
                    ...
                    <div class="reveal-card bg-black rounded-lg">
                        <button @click="open = (open === 1 ? 0 : 1)" class="w-full text-left p-4">
                            <div class="flex justify-between items-center">
                                ...
                                <i data-lucide="chevron-down" class="transition-transform" :class="{ 'rotate-180': open === 1 }"></i>
                            </div>
                        </button>
                    ...
                    <div class="reveal-card bg-black rounded-lg">
                        <button @click="open = (open === 2 ? 0 : 2)" class="w-full text-left p-4">
                            <div class="flex justify-between items-center">
                                ...
                                <i data-lucide="chevron-down" class="transition-transform" :class="{ 'rotate-180': open === 2 }"></i>
                            </div>
                        </button>
                    ...
                    <div class="reveal-card bg-black rounded-lg">
                        <button @click="open = (open === 3 ? 0 : 3)" class="w-full text-left p-4">
                            <div class="flex justify-between items-center">
                                ...
                                <i data-lucide="chevron-down" class="transition-transform" :class="{ 'rotate-180': open === 3 }"></i>
                            </div>
                        </button>
        ...
        <section id="custom-order" class="py-20 sm:py-24 bg-gray-900">
            <div class="container mx-auto px-4 sm:px-6">
                <div class="text-center mb-12">
                    <h2 class="text-3xl sm:text-4xl font-bold text-white">Pesan Produk Custom</h2>
                    <p class="text-gray-400 mt-2">Buat produk sesuai keinginanmu dengan mudah.</p>
                </div>

                <form action="{{ route('custom.order') }}" method="POST" enctype="multipart/form-data" class="max-w-3xl mx-auto bg-black p-8 rounded-lg shadow-lg">
                    @csrf
                    <div class="mb-6">
                        <label for="name" class="block text-sm font-medium text-gray-400 mb-2">Nama Produk</label>
                        <input type="text" id="name" name="name" class="w-full px-4 py-2 text-sm text-gray-900 bg-gray-800 rounded-lg focus:outline-none focus:ring-2 focus:ring-yellow-500" placeholder="Masukkan nama produk" required>
                    </div>

                    <div class="mb-6">
                        <label for="description" class="block text-sm font-medium text-gray-400 mb-2">Deskripsi</label>
                        <textarea id="description" name="description" rows="4" class="w-full px-4 py-2 text-sm text-gray-900 bg-gray-800 rounded-lg focus:outline-none focus:ring-2 focus:ring-yellow-500" placeholder="Deskripsikan produk custom Anda" required></textarea>
                    </div>

                    <div class="mb-6">
                        <label for="image" class="block text-sm font-medium text-gray-400 mb-2">Unggah Desain</label>
                        <input type="file" id="image" name="image" class="w-full text-sm text-gray-400 bg-gray-800 rounded-lg focus:outline-none focus:ring-2 focus:ring-yellow-500" required>
                    </div>

                    <div class="mb-6">
                        <label for="quantity" class="block text-sm font-medium text-gray-400 mb-2">Jumlah</label>
                        <input type="number" id="quantity" name="quantity" class="w-full px-4 py-2 text-sm text-gray-900 bg-gray-800 rounded-lg focus:outline-none focus:ring-2 focus:ring-yellow-500" placeholder="Masukkan jumlah pesanan" required>
                    </div>
